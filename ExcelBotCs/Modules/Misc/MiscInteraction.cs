...
public class MiscInteraction : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task RandomMelee()
    {
        await RandomJob("Dragoon", "Monk", "Ninja", "Samurai", "Reaper", "Viper");
    }

    [SlashCommand("randomcaster", "Gives you a random Magical Ranged DPS job")]
    public async Task RandomCaster()
    {
        await RandomJob("Black Mage", "Summoner", "Red Mage", "Pictomancer");
    }

    [SlashCommand("randomranged", "Gives you a random Physical Ranged DPS job")]
    public async Task RandomRanged()
    {
        await RandomJob("Bard", "Machinist", "Dancer");
    }

    [SlashCommand("randomhealer", "Gives you a random Healer job")]
    public async Task RandomHealer()
    {
        await RandomJob("White Mage", "Scholar", "Astrologian", "Sage");
    }

    [SlashCommand("randomtank", "Gives you a random Tank job")]
    public async Task RandomTank()
    {
        await RandomJob("Paladin", "Warrior", "Dark Knight", "Gunbreaker");
    }

    [SlashCommand("randomjob", "Gives you a random job")]
    public async Task RandomJob()
    {
        await RandomJob("Dragoon", "Monk", "Ninja", "Samurai", "Reaper", "Viper",
